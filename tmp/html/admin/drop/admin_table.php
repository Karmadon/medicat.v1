...
$id = (int)$_GET['id'];
$sql = "SELECT groupd_id FROM users WHERE id='$id'";
$result = $conn->query($sql);
$row = $result->fetch_assoc();
if ($row["groupd_id"] =='101') {
echo'<!-- SELECT2 EXAMPLE -->
	  <form role="form" method="POST" action="tmp/html/admin/drop/drop.php?id='.$id.'">
      <div class="box box-default">
        <div class="box-body">
		<div class="alert alert-danger alert-dismissible">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                <h4><i class="icon fa fa-ban"></i> Внимание!</h4>
                Если Вы удалите менеджера, то в модуле "запись килентов" поле "К какому менеджеру обратились?", получит статус "Неизвестно".
              </div>
