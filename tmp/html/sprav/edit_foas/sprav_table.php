...
$id = (int)$_GET['id'];
$sql = "SELECT * FROM sprav_foas WHERE id='$id'";
$result = $conn->query($sql);
$row = $result->fetch_assoc();

echo'<!-- SELECT2 EXAMPLE -->
	  <form role="form" method="POST" action="tmp/html/sprav/edit_foas/edit.php?id='.$id.'">
		...
		<div class="row">
            <div class="col-md-12">
              <div class="form-group">
                ...
                <input type="text" name="name" class="form-control" value="'.$row["name"].'" placeholder="Откуда узнали о нас" required>
              </div>
              <!-- /.form-group -->
            </div>
            <!-- /.col -->
