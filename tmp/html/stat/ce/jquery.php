...
<section class="content">
	...
	$sql_full = "SELECT COUNT(*) as `id` FROM `ce` WHERE `date` >= '".$type_f."' AND `date` <= '".$type_e."'";
	...
	$result_count_data = 1;
	$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE `date` = '".$ft."'";
	...
	$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE `date` >= '".$ft."' AND `date` <= '".$st."'";
	$result = $conn->query($sql);
	$row = $result->fetch_assoc();
	$count_one = $row['id'];
	}
	...
	while ($x++ < $result_count_data) { # Запускаем цикл
		...
		if ($x == 0) {
			$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE `date` >= '".$ft."' AND `date` <= '".$data_s[$x]."'"; # Поиск первого месяца из диапазона
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_first = $row['id'];
			$count_f[]= $count_first; 
		...
		elseif($x > 0 && $x < $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE `date` >= '".$data_f[$x]."' AND `date` <= '".$data_s[$x]."'"; # Поиск последующих месяцев из диапазона
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_many = $row['id'];
			$count_m[]= $count_many; 
		...
		elseif($x == $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE `date` >= '".$data_f[$x]."' AND `date` <= '".$st."'"; # Поиск последнего месяца из диапазона
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_end = $row['id'];
			$count_e[]= $count_end; 
	...
	?>
                    ...
                    <div class="box-body">
                        ...
                        <div class="disabled">
                            <p class="text-muted well well-sm no-shadow" style="margin-top: 10px;">
                                ...
                                было записано <?echo $full; ?> клиентов
                            </p>
            ...
            <div class="col-md-12">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        <?
	   $wia_find = "SELECT wia, count(*) FROM ce WHERE `date` >= '".$type_f."' AND `date` <= '".$type_e."' GROUP BY wia HAVING count(*) >= 1 ORDER BY count(*)";
	   ...
	   $sql4 = "SELECT * FROM sprav_wia WHERE id=".$row_find_wia["wia"]."";
            ...
            <div class="col-md-12">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        <?
	   $twtma_find = "SELECT twtma, count(*) FROM ce WHERE `date` >= '".$type_f."' AND `date` <= '".$type_e."' GROUP BY twtma HAVING count(*) >= 1 ORDER BY count(*)";
	   ...
	   $sql5 = "SELECT * FROM users WHERE id=".$row_find_twtma["twtma"]."";
...
/* ChartJS
	...
	var areaChartData2 = {
      ...
      datasets: [
		...
		if ($ft==$st) {# Если выборка за 1 день
			...
			$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE wia=".$row_find_wia["wia"]." AND `date` = '".$ft."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_one = $row['id'];
	
			$sql44 = "SELECT * FROM sprav_wia WHERE id=".$row_find_wia["wia"]."";
	...
	if ($result_count_data == 1) { # Проверяем находится ли диапазон в одном месяце
			...
			$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE wia=".$row_find_wia["wia"]." AND `date` >= '".$ft."' AND `date` <= '".$st."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_one = $row['id'];
			...
			while ($x++ < $result_count_data) { # Запускаем цикл
			if ($x == 0) {
			$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE wia=".$row_find_wia["id"]." AND `date` >= '".$ft."' AND `date` <= '".$data_s[$x]."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_first = $row['id'];
			$count_f[]= $count_first;
			...
			elseif($x > 0 && $x < $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE wia=".$row_find_wia["id"]." AND `date` >= '".$data_f[$x]."' AND `date` <= '".$data_s[$x]."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_many = $row['id'];
			$count_m[]= $count_many; 
			...
			elseif($x == $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE wia=".$row_find_wia["id"]." AND `date` >= '".$data_f[$x]."' AND `date` <= '".$st."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_end = $row['id'];
			$count_e[]= $count_end; 
...
?>
	...
	var areaChartData3 = {
      ...
      datasets: [
        ...
        <?php
		$sql_find_twtma  = "SELECT * FROM users WHERE id != 1";	
		$result_find_twtma = $conn->query($sql_find_twtma);
		...
		if ($ft==$st) {# Если выборка за 1 день
			...
			$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE twtma=".$row_f_twtma["twtma"]." AND `date` = '".$ft."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_one = $row['id'];
	
			$sql44 = "SELECT * FROM users WHERE id=".$row_f_twtma["twtma"]."";
			$result44 = $conn->query($sql44);
			$row44 = $result44->fetch_assoc();
	...
	if ($result_count_data == 1) { # Проверяем находится ли диапазон в одном месяце
			...
			$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE twtma=".$row_f_twtma["twtma"]." AND `date` >= '".$ft."' AND `date` <= '".$st."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_one = $row['id'];
			...
			while($row_find_twtma = $result_find_twtma->fetch_assoc()) {
			$sql44 = "SELECT * FROM users WHERE id=".$row_find_twtma["id"]."";
			$result44 = $conn->query($sql44);
			$row44 = $result44->fetch_assoc();	
			$count_f = array(); # Массив первый месяц
			$count_m = array(); # Массив последующие месяцы
			...
			while ($x++ < $result_count_data) { # Запускаем цикл
			if ($x == 0) {
			$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE twtma=".$row_find_twtma["id"]." AND `date` >= '".$ft."' AND `date` <= '".$data_s[$x]."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_first = $row['id'];
			$count_f[]= $count_first;
			...
			elseif($x > 0 && $x < $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE twtma=".$row_find_twtma["id"]." AND `date` >= '".$data_f[$x]."' AND `date` <= '".$data_s[$x]."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_many = $row['id'];
			$count_m[]= $count_many; 
			...
			elseif($x == $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `ce` WHERE twtma=".$row_find_twtma["id"]." AND `date` >= '".$data_f[$x]."' AND `date` <= '".$st."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_end = $row['id'];
			$count_e[]= $count_end; 
