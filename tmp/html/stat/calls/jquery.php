...
<section class="content">
	...
	$sql_full = "SELECT COUNT(*) as `id` FROM `calls` WHERE `date` >= '".$type_f."' AND `date` <= '".$type_e."'";
	...
	$result_count_data = 1;
	$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE `date` = '".$ft."'";
	...
	$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE `date` >= '".$ft."' AND `date` <= '".$st."'";
	$result = $conn->query($sql);
	$row = $result->fetch_assoc();
	$count_one = $row['id'];
	}
	...
	while ($x++ < $result_count_data) { # Запускаем цикл
		...
		if ($x == 0) {
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE `date` >= '".$ft."' AND `date` <= '".$data_s[$x]."'"; # Поиск первого месяца из диапазона
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_first = $row['id'];
			$count_f[]= $count_first; 
		...
		elseif($x > 0 && $x < $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE `date` >= '".$data_f[$x]."' AND `date` <= '".$data_s[$x]."'"; # Поиск последующих месяцев из диапазона
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_many = $row['id'];
			$count_m[]= $count_many; 
		...
		elseif($x == $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE `date` >= '".$data_f[$x]."' AND `date` <= '".$st."'"; # Поиск последнего месяца из диапазона
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_end = $row['id'];
			$count_e[]= $count_end; 
            ...
            <div class="col-md-12">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        <?
	   $foas_find = "SELECT foas, count(*) FROM calls WHERE `date` >= '".$type_f."' AND `date` <= '".$type_e."' GROUP BY foas HAVING count(*) >= 1 ORDER BY count(*)";
		  ...
		  <div class="row">
            <div class="col-md-12">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        <div class="chart">
                            <canvas id="lineChart_period2" style="height:250px"></canvas>
                        </div>
                        <?
	   $oapc_find = "SELECT oapc, count(*) FROM calls WHERE `date` >= '".$type_f."' AND `date` <= '".$type_e."' GROUP BY oapc HAVING count(*) >= 1 ORDER BY count(*)";
		...
		<div class="row">
            <div class="col-md-12">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        <div class="chart">
                            <canvas id="lineChart_period3" style="height:250px"></canvas>
                        </div>
                        <?
	   $wia_find = "SELECT wia, count(*) FROM calls WHERE `date` >= '".$type_f."' AND `date` <= '".$type_e."' GROUP BY wia HAVING count(*) >= 1 ORDER BY count(*)";
...
/* ChartJS
	...
	var areaChartData2 = {
      ...
      datasets: [
        ...
        <?php
		$sql_find_foas  = "SELECT * FROM sprav_foas WHERE id != 0";	
		$result_find_foas = $conn->query($sql_find_foas);
		...
		if ($ft==$st) {# Если выборка за 1 день
			...
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE foas=".$row_f_foas["foas"]." AND `date` = '".$ft."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_one = $row['id'];
	...
	if ($result_count_data == 1) { # Проверяем находится ли диапазон в одном месяце
			...
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE foas=".$row_f_foas["foas"]." AND `date` >= '".$ft."' AND `date` <= '".$st."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_one = $row['id'];
			...
			while($row_find_foas = $result_find_foas->fetch_assoc()) {
			$sql44 = "SELECT * FROM sprav_foas WHERE id=".$row_find_foas["id"]."";
			$result44 = $conn->query($sql44);
			$row44 = $result44->fetch_assoc();	
			$count_f = array(); # Массив первый месяц
			$count_m = array(); # Массив последующие месяцы
			...
			while ($x++ < $result_count_data) { # Запускаем цикл
			if ($x == 0) {
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE foas=".$row_find_foas["id"]." AND `date` >= '".$ft."' AND `date` <= '".$data_s[$x]."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_first = $row['id'];
			$count_f[]= $count_first;
			...
			elseif($x > 0 && $x < $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE foas=".$row_find_foas["id"]." AND `date` >= '".$data_f[$x]."' AND `date` <= '".$data_s[$x]."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_many = $row['id'];
			$count_m[]= $count_many; 
			...
			elseif($x == $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE foas=".$row_find_foas["id"]." AND `date` >= '".$data_f[$x]."' AND `date` <= '".$st."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_end = $row['id'];
			$count_e[]= $count_end; 
...
?>
	...
	var areaChartData3 = {
      ...
      datasets: [
        ...
        <?php
		$sql_find_oapc  = "SELECT * FROM sprav_phone WHERE id != 0";	
		$result_find_oapc = $conn->query($sql_find_oapc);
		...
		if ($ft==$st) {# Если выборка за 1 день
			...
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE oapc=".$row_f_oapc["oapc"]." AND `date` = '".$ft."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_one = $row['id'];
	
			$sql44 = "SELECT * FROM sprav_phone WHERE id=".$row_f_oapc["oapc"]."";
			$result44 = $conn->query($sql44);
			$row44 = $result44->fetch_assoc();
	...
	if ($result_count_data == 1) { # Проверяем находится ли диапазон в одном месяце
			...
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE oapc=".$row_f_oapc["oapc"]." AND `date` >= '".$ft."' AND `date` <= '".$st."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_one = $row['id'];
			...
			while($row_find_oapc = $result_find_oapc->fetch_assoc()) {
			$sql44 = "SELECT * FROM sprav_phone WHERE id=".$row_find_oapc["id"]."";
			$result44 = $conn->query($sql44);
			$row44 = $result44->fetch_assoc();	
			$count_f = array(); # Массив первый месяц
			$count_m = array(); # Массив последующие месяцы
			...
			while ($x++ < $result_count_data) { # Запускаем цикл
			if ($x == 0) {
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE oapc=".$row_find_oapc["id"]." AND `date` >= '".$ft."' AND `date` <= '".$data_s[$x]."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_first = $row['id'];
			$count_f[]= $count_first;
			...
			elseif($x > 0 && $x < $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE oapc=".$row_find_oapc["id"]." AND `date` >= '".$data_f[$x]."' AND `date` <= '".$data_s[$x]."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_many = $row['id'];
			$count_m[]= $count_many; 
			...
			elseif($x == $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE oapc=".$row_find_oapc["id"]." AND `date` >= '".$data_f[$x]."' AND `date` <= '".$st."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_end = $row['id'];
			$count_e[]= $count_end; 
...
?>
	...
	var areaChartData4 = {
      ...
      datasets: [
        ...
        <?php
		$sql_find_wia  = "SELECT * FROM sprav_wia WHERE id != 0";	
		$result_find_wia = $conn->query($sql_find_wia);
		...
		if ($ft==$st) {# Если выборка за 1 день
			...
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE wia=".$row_f_wia["wia"]." AND `date` = '".$ft."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_one = $row['id'];
	
			$sql44 = "SELECT * FROM sprav_wia WHERE id=".$row_f_wia["wia"]."";
			$result44 = $conn->query($sql44);
			$row44 = $result44->fetch_assoc();
	...
	if ($result_count_data == 1) { # Проверяем находится ли диапазон в одном месяце
			...
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE wia=".$row_f_wia["wia"]." AND `date` >= '".$ft."' AND `date` <= '".$st."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_one = $row['id'];
			...
			while($row_find_wia = $result_find_wia->fetch_assoc()) {
			$sql44 = "SELECT * FROM sprav_wia WHERE id=".$row_find_wia["id"]."";
			$result44 = $conn->query($sql44);
			$row44 = $result44->fetch_assoc();	
			$count_f = array(); # Массив первый месяц
			$count_m = array(); # Массив последующие месяцы
			...
			while ($x++ < $result_count_data) { # Запускаем цикл
			if ($x == 0) {
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE wia=".$row_find_wia["id"]." AND `date` >= '".$ft."' AND `date` <= '".$data_s[$x]."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_first = $row['id'];
			$count_f[]= $count_first;
			...
			elseif($x > 0 && $x < $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE wia=".$row_find_wia["id"]." AND `date` >= '".$data_f[$x]."' AND `date` <= '".$data_s[$x]."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_many = $row['id'];
			$count_m[]= $count_many; 
			...
			elseif($x == $final) {
			$sql = "SELECT COUNT(*) as `id` FROM `calls` WHERE wia=".$row_find_wia["id"]." AND `date` >= '".$data_f[$x]."' AND `date` <= '".$st."'";
			$result = $conn->query($sql);
			$row = $result->fetch_assoc();
			$count_end = $row['id'];
			$count_e[]= $count_end; 
...
?>
    ...
    //- LINE CHART -
    //--------------
    var lineChartCanvas2 = $("#lineChart_period2").get(0).getContext("2d");
    ...
    //- LINE CHART -
    //--------------
    var lineChartCanvas3 = $("#lineChart_period3").get(0).getContext("2d");
